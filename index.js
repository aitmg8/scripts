...
async function getGenerativeModel() {
  ...
  return vertexAI.getGenerativeModel({ model: 'gemini-1.5-flash-001' });
}
...
exports.organizeFile = async (file, context) => {
  ...
  try {
    ...
    const apiKeys = await getApiKeys(); // keys for Qdrant check
...
// Prefer secret-backed env vars; if missing, fall back to Secret Manager API
async function getApiKeys() {
  const maybe = (k) => process.env[k] && String(process.env[k]).trim();

  const openAIEnv = maybe('OPENAI_API_KEY');
  const geminiEnv = maybe('GEMINI_API_KEY');
  const qdrantKeyEnv = maybe('QDRANT_API_KEY');
  const qdrantUrlEnv = maybe('QDRANT_API_URL');

  if (openAIEnv && geminiEnv && qdrantKeyEnv && qdrantUrlEnv) {
    return {
      openAIKey: openAIEnv,
      geminiKey: geminiEnv,
      qdrantKey: qdrantKeyEnv,
