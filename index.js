...
// Resolve project id reliably on Cloud Functions (gen2) / Cloud Run
async function getProjectId() {
  return (
    process.env.GOOGLE_CLOUD_PROJECT ||
    process.env.GCLOUD_PROJECT ||
    (await new GoogleAuth().getProjectId())
  );
}

// Vertex AI lazy init so we don't pin a wrong project id
...
async function getGenerativeModel() {
  ...
  return vertexAI.getGenerativeModel({ model: 'gemini-1.5-flash-001' }); // gemini-1.5-flash-001
}
...
const WF_CONFIG = {
  ...
  ERROR_BUCKET_NAME: 'files-in-error',
...
exports.organizeFile = async (file, context) => {
  ...
  try {
    ...
    const apiKeys = getApiKeys(); // from env vars (no Secret Manager calls)
    ...
    const suggestedBaseName =
      result.response?.candidates?.[0]?.content?.parts?.[0]?.text?.trim()?.replace(/["']/g, '');
    ...
    // --- 4. Move to Organized Bucket with new name ---
    await sourceFile.move(storage.bucket(ORGANIZE_CONFIG.ORGANIZED_BUCKET).file(newName));
...
exports.processFileToQdrant = async (file, context) => {
  ...
  try {
    const apiKeys = getApiKeys(); // env only
    ...
    const chunkPoints = createChunkPoints(
      fileId,
      sourceFileName,
      chunks,
      openaiEmbeddings,
    ...
    await storage
      ...
      .file(sourceFileName)
      .move(storage.bucket(WF_CONFIG.COMPLETED_BUCKET_NAME).file(sourceFileName));
    console.log(`[Indexer] ✅ Successfully processed and moved ${sourceFileName}.`);
  } catch (error) {
    console.error(
      `[Indexer] ❌ Failed to process file ${sourceFileName}:`,
      error.response ? error.response.data : error
    );
    await storage
      .bucket(sourceBucketName)
      .file(sourceFileName)
      .move(storage.bucket(WF_CONFIG.ERROR_BUCKET_NAME).file(sourceFileName));
  }
};
...
// Read secrets exclusively from env vars that were populated via --set-secrets
function getApiKeys() {
  const need = (k) => {
    const v = process.env[k];
    if (!v) throw new Error(`Missing required env var: ${k}. Did you deploy with --set-secrets ${k}=... ?`);
    return String(v).trim();
  };
  return {
    openAIKey: need('OPENAI_API_KEY'),
    geminiKey: need('GEMINI_API_KEY'),
    qdrantKey: need('QDRANT_API_KEY'),
    qdrantUrl: need('QDRANT_API_URL'),
  };
}
...
async function ensureQdrantIndexes(apiKeys) {
  ...
  try {
    ...
    if (error.response?.status !== 409) {
      console.error(
        `[Indexer] ⚠️ Could not create Qdrant payload index:`,
...
async function getGeminiEmbeddings(chunks, apiKey) {
  ...
  const requests = chunks.map((chunk) => ({
    model: `models/${WF_CONFIG.EMBEDDING_MODEL_GEMINI}`,
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a meticulous document analyst. Summarize the following text, capturing all key details, figures, and names precisely.',
          },
          { role: 'user', content: chunk },
        ],
        temperature: WF_CONFIG.SUMMARY_TEMPERATURE,
        max_tokens: WF_CONFIG.SUMMARY_MAX_TOKENS,
      },
      { headers: { Authorization: `Bearer ${apiKey}` } }
    )
  );
...
async function getDocSummary(chunkSummaries, apiKey) {
  ...
  const response = await axios.post(
    'https://api.openai.com/v1/chat/completions',
    {
      model: WF_CONFIG.SUMMARY_MODEL,
      messages: [
        { role: 'system', content: 'You are a senior analyst. Write a comprehensive, well-structured document-level summary.' },
